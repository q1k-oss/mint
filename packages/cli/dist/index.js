...
function writeOutput(content, outputPath) {
  ...
  console.error(`\u{1F33F} Written to: ${outputPath}`);
}
function detectMode(inputPath, options) {
...
function showStats(input, output, mode) {
...
function main() {
  ...
  if (options.version) {
    console.log(`\u{1F33F} MINT Format v${VERSION}`);
    process.exit(0);
  }
  try {
    ...
    if (options.validate) {
      ...
      if (result.valid) {
        console.log("\u{1F33F} \u2713 Valid MINT format");
        process.exit(0);
      } else {
